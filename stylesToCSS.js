...
var _unsupportedPseudoClasses = require('./unsupportedPseudoClasses');

var _unsupportedPseudoClasses2 = _interopRequireDefault(_unsupportedPseudoClasses);
...
function buildRule(result, key, value, selector) {
  if (!_react.isUnitlessNumber[key] && typeof value === 'number') {
    value = '' + value + 'px';
  } else if (key === 'content' && !unquotedContentValueRegex.test(value)) {
    value = "'" + value.replace(/'/g, "\\'") + "'";
  }

  result.css += '  ' + (0, _react.hyphenateStyleName)(key) + ': ' + value + ';\n';
}
...
function buildRules(result, rules, uniqueKey, selector, maxLength, key) {
  ...
  for (var j = 0, l = styleKeys.length; j < l; j++) {
    ...
    if (_unsupportedPseudoClasses2.default[styleKey.split('(')[0].trim()]) {
